...
switch($ts){
	...
	case "":
		...
		$url = SITE_URL.tsUrl('group','cate',array('page'=>''));
		$lstart = $page*20-20;
		$arrGroup = $new['group']->findAll('group',null,null,null,$lstart.',20');
	...
	case "2":
		...
		//分类下小组
		$page = isset($_GET['page']) ? $_GET['page'] : '1';
		$url = SITE_URL.tsUrl('group','cate',array('ts'=>'2','page'=>''));
		$lstart = $page*20-20;
	...
	case "3":
		...
		//上级分类
		$oneCate = $new['group']->find('group_cates',array(
		...
		//分类下小组
		$page = isset($_GET['page']) ? $_GET['page'] : '1';
		$url = SITE_URL.tsUrl('group','cate',array('ts'=>'3','page'=>''));
		$lstart = $page*20-20;
		$arrGroup = $new['group']->findAll('group',array(
			'cateid2'=>$cateid,
		),null,null,$lstart.',20');
	...
	case "group":
		...
		$twoCate = $new['group']->find('group_cates',array(
			'cateid'=>$strCate['referid'],
		));
	...
	case "two":
		$cateid = $_GET['cateid'];
		$arrCate = $db->fetch_all_assoc("select * from ".dbprefix."group_cates where referid='$cateid'");
	...
	//三级分类
	case "three":
		$cateid2 = $_GET['cateid2'];
		$arrCate = $db->fetch_all_assoc("select * from ".dbprefix."group_cates where referid='$cateid2'");
