...
class i18n {
	...
	public function __construct($filePath = NULL, $cachePath = NULL, $fallbackLang = NULL, $forcedLang = NULL, $sectionSeperator = NULL) {
		...
		/*QiuJun create dir start*/
		if(!is_dir($cachePath)){
			createFolders($cachePath);
		}
		/*QiuJun create dir end*/
    ...
    public function init() {
        ...
        $this->cacheFilePath = $this->cachePath . '/' .$app. '_' . $this->appliedLang . '.php';
        ...
        if(!file_exists($this->cacheFilePath) || filemtime($this->cacheFilePath) < filemtime($this->langFilePath)) {
			...
			if(is_file('public/lang/lang_'.$this->appliedLang.'.ini')){
				$pub_ini = parse_ini_file('public/lang/'.$this->appliedLang.'.ini', true);
			}else{
				$pub_ini = parse_ini_file('public/lang/'.$this->appliedLang.'.ini', true);
			}
			/*添加结束*/
